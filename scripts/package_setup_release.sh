...
#
# Writes: dist/fft-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix fft \
  --exe-name Final_Fantasy_Tactics_Recompiled \
  --display-name "Final Fantasy Tactics Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Final Fantasy Tactics USA disc" \
  ...
  --project-file RENDERING.md \
  --project-file VERIFICATION.md \
  --project-file keybinds.ini \
  --project-file gamecontrollerdb.txt \
  --project-dir seeds \
  --project-dir LICENSES \
  "${EXTRA_PROJECT[@]}"
